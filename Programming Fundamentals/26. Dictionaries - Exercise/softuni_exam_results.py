...
while info != "exam finished":
	if "banned" in info:
		student, action = info.split("-")
		results.pop(student, None)
	else:
		student, language, points = info.split("-")
		points = int(points)
		if student not in results:
			results[student] = {}
		if language not in results[student] or points > results[student][language]:
			results[student][language] = points
