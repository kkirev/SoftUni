...
It can only receive users' data, but it doesn't know what to do with it. Good thing you're on the dev team and know
how to fix it, right?
...
for movement in range(commands_number):
	command = input().split()
	action = command[0]
	username = command[1]
	...
	if action == "register":
		...
		if username not in parking:
			parking.update({username: car_number})
...
for username, car_number in parking.items():
	print(f"{username} => {car_number}")
