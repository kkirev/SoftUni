...
You will receive a string with even integers, separated by a "@" - this is our neighborhood. After that,
a series of Jump commands will follow until you receive "Love!". Every house in the neighborhood needs a certain number
of hearts delivered by Cupid so it can celebrate Valentine's day.
...
neighborhood_numbers = [int(number) for number in input().split("@")]
command = input()
current_house_index = 0

while command != "Love!":
	house_index = command.split()[1]
	house_index = int(house_index)

	if house_index > len(neighborhood_numbers):
		current_house_index = 0
	else:
		neighborhood_numbers[house_index] -= 2
		current_house_index += house_index

	if neighborhood_numbers[current_house_index] == 0:
...
for house in neighborhood_numbers:
	zero_counter = 0
	if house == 0:
		zero_counter += 1
if zero_counter == len(neighborhood_numbers):
	print(f"Mission was successful.")
else:
	print(f'"Cupid has failed {len(neighborhood_numbers) - zero_counter} places."')
